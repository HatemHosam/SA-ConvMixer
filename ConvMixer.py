...
def get_SA_Convmixer_ADE(
    image_size=320, filters=512, depth=16, kernel_size=5, patch_size=5, out_channels=150, SR = False, dataset = 'ADE20K'):
...
def get_SA_Convmixer(
    image_size=320, filters=512, depth=12, kernel_size=5, patch_size=5, out_channels=21, SR = False, dataset = 'VOC'):

    inputs = keras.Input((image_size, image_size, 3))
    
    if dataset = 'VOC' or dataset = 'Cityscapes-seg' or dataset = 'ADE20K':
        x = tf.keras.layers.Normalization(mean=[0.485, 0.456, 0.406], variance= [0.229**2, 0.224**2, 0.225**2])(inputs)
    else:
